...
專業的加密貨幣套利交易平台，支援跨交易所的差價套利和TWAP（時間加權平均價格）執行策略。
...
# 3. 啟動服務
# 終端 1: 啟動後端
cd python_backend && python -m uvicorn app.main:app --host 0.0.0.0 --port 7001 --reload
...
2. **端口佔用**：檢查端口 3000 和 7001 是否被佔用
3. **API 密鑰**：確保 `.env` 文件中的 API 密鑰格式正確
4. **依賴安裝**：確保 Python 和 Node.js 依賴都已正確安裝

### Q: 如何查看錯誤日誌？
A: 查看終端輸出中的錯誤信息，或檢查瀏覽器控制台（F12）

### Q: 如何檢查系統狀態？
A: 訪問以下端點檢查系統狀態：
- **健康檢查**：http://localhost:7001/health
- **配置狀態**：http://localhost:7001/api/config/status
- **前端界面**：http://localhost:3000

### Q: 如何停止服務？
A: 在運行服務的終端按 `Ctrl+C` 停止服務

### Q: 如何重新配置？
A: 編輯 `.env` 文件後重啟服務（停止後重新運行啟動命令）
